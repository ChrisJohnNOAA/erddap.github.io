...
Pas 4. Reporni apache2: `sudo systemctl restart apache2` 
...
Linia 2 `RescrieCond % &#123;QUERY_STRING&#125; ^$` 
O condiție care trebuie să fie adevărată înainte de aplicarea regulii de mai jos. `% &#123;QUERY_STRING&#125;` Este totul după? în URL-ul cererii. ^$ este un regex care înseamnă "start de șir imediat urmat de sfârșitul șir" . Adică, un șir gol. Deci, această condiție este adevărat numai atunci când nu există nici un șir de întrebări la toate 

Linia 3 `RescrieCond % &#123;REQUEST_URI&#125; ^/erddap/ (griddap |  tabledap ) / [^/?]+\\. (&#33; (?:...) $) [A-Za-z0-9_]+$` 
O a doua condiție, verificat împotriva `% &#123;REQUEST_URI&#125;`  (ales în mod deliberat pentru a lăsa modelul RescrieRegula în sine face potrivire, deoarece model-potrivire în interiorul unui ` <Location> ` blocul se poate comporta ambiguu ) . Spargerea regex:

 `^/erddap/` Trebuie să înceapă cu /erddap /
...
Linia 4 `Rescrierea regulii ^ - [R=429,L]` 
Regula în sine. Deoarece ambele condiții de mai sus trebuie să fie deja adevărat pentru Apache pentru a evalua chiar și această linie, modelul aici nu are nevoie pentru a verifica nimic altceva  - înseamnă "nu rescrie URL-ul la nimic diferit" (Nu redirecţionăm nicăieri, doar scurtcircuităm cererea.) . Steagurile:
 ...
 `R=429` Raspundeti cu un cod de stare HTTP redirection-class ("Prea multe cereri") în loc de a servi cererea.
L 
...
De ce? `% &#123;REQUEST_URI&#125;` în loc de a lăsa modelul de rescriere se potrivesc direct calea (în valoare de inclusiv ca o notă pentru colegi, deoarece este partea non-evident) : în interiorul ` <Location> ` block, ceea ce un model de rescriere gol se potriveste de fapt împotriva poate comporta inconsecvent în funcție de versiunea Apache și context. trăgând explicit calea completă prin RescrieCond `% &#123;REQUEST_URI&#125;` Sidesteps care ambiguitate în întregime     
