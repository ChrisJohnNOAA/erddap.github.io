...
```
* Utilizare `python translation/translate.py` actualizarea traducerilor, dacă este necesar.
...
* Șterge dosarul i18n/en (necesare deoarece scrie-translations nu va suprascrie un fișier existent) şi fugi. `npm run write-translations` pentru a exporta noul şir de picioare.
* Editează paginile de documentare (implementarea-instalare.md și implementarea update.md) .
  * Caută \\[ erddap.war \\]  
  * Copiază informațiile existente (ușor reformat) la lista instalațiilor anterioare.
  * Schimbă informațiile de eliberare curentă pentru Erddap. război la \\[ erddap.war \\] 
* Rulați traducerile pentru site-ul de documentare. Se recomandă doar traducerea paginilor care s-au schimbat, deoarece acest pas poate fi foarte lent.
  * Asigurați-vă că picioarele sunt traduse cu noul număr versiune.
