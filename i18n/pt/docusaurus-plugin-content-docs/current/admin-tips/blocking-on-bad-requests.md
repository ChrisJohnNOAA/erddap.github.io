...
Reescrever Engine Em
RewriteCond %&#123;QUERY_STRING&#125;
RewriteCond %&#123;REQUEST_URI&#125; ^/erddap/ (Anúncio grátis para sua empresa |  tabledap ) - Sim. (?&#33; (:croissant | INSTITUIÇÕES | iso19139_2007 | iso19115_3_2016 | ncCFHeader | ncCFMAAquecedor | das | Dds | html | gráfico | subconjunto | NcHeader | Ajudar | Fgdc | INSTITUIÇÕES | ncoJsonHeader) $) [A-Za-z0-9_]
RewriteRule ^ - [R=429,L]

Passo 3. Verifique se a configuração é válida: sudo apache2ctl configtest

Passo 4. Reinicie apache2: sudo systemctl restart apache2
...
Linha de linha 4 — RewriteRule ^ - [R=429,L]
...
R=429 — responda com uma ação de redirecionamento HTTP que carrega o código de status 429 ("Muitos pedidos") em vez de servir o pedido.
...
Por que %&#123;REQUEST_URI&#125; em vez de deixar o padrão RewriteRule corresponder diretamente ao caminho (vale a pena incluir como uma nota para colegas, uma vez que é a parte não-obvious) : dentro de um <Location> block, o que um padrão RewriteRule nu realmente é combinado com pode se comportar inconsistentemente dependendo da versão e contexto do Apache. Explicitamente puxando o caminho completo através RewriteCond %&#123;REQUEST_URI&#125; sidesteps que a ambiguidade inteiramente — é sempre o caminho de solicitação literal, completo, de modo que o regex se comporta exatamente como escrito independentemente de onde a regra está aninhada.
