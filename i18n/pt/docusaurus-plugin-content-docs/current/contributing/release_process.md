...
```
* Uso `tradução python / tradução.py` para atualizar traduções se necessário.
...
* Excluir o diretório i18n/en (necessário porque as translações de gravação não substituirão um arquivo existente) e correr `npm run write-translations` para exportar a nova cadeia de rodapé.
* Editar as páginas de documentação (deploy-install.md e deploy-update.md) .
  * Pesquisar \\[ erddap.war \\]  
  * Copie as informações existentes (ligeiramente reformatado) para a lista de instalações anteriores.
  * Altere as informações de lançamento atuais para erddap. em guerra \\[ erddap.war \\] 
* Execute as traduções para o site de documentação. Recomenda-se apenas traduzir páginas que mudaram desde que este passo pode ser muito lento.
  * Certifique-se de que os rodapés são traduzidos com o novo número de versão.
