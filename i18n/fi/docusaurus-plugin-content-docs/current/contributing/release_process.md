...
```
* Käytä `python translation/translate.py` käännökset päivitetään tarvittaessa.
...
* Poista i18n/en-hakemisto (Tarvitaan, koska kirjalliset käännökset eivät ylitä olemassa olevaa tiedostoa.) ja juoksemaan `npm käännökset` viemään uutta merkkijonoa.
* Muokkaa dokumentointisivuja (käyttöönotto-install.md ja käyttöönotto-update.md) .
  * Etsiminen \\[ Erddap.sota \\]  
  * Kopioi olemassa olevat tiedot (Vähän uudistuksia) aiempien laitosten luetteloon.
  * Vaihda nykyinen tiedote erddapin. sotaa vastaan \\[ Erddap.sota \\] 
* Käynnistä käännökset dokumentointisivustolle. On suositeltavaa kääntää sivuja, jotka ovat muuttuneet, koska tämä vaihe voi olla hyvin hidas.
  * Varmista, että käännökset on käännetty uudella versiolla.
