...
*    **Uudet ominaisuudet ja muutokset (Käyttäjille) :** 
      * Uusi ncoJson infotulomuodoksi, joka on saatavana tiedostona nimeltä nco JsonHeader ja molemmat /erddap/info/dataset I .nc OJson ja /erddap/info/datasetId/indeksi .nc OJson.
Kiitos @srstsavage
      * ISO19139_2007:n ja ISO19115_3_2016:n lisäominaisuudet: luokitus, aihepiiri ja ylläpito_taajuus
Kiitos @SethChampagneNRL
      * Uusi JSON-verkkopalvelu vaihteleville attribuuttiehdotuksille: /erddap/suggestVariableAttributes
Kiitos @srstsavage
      * Parannettu sql-pohjaisten tietoaineistojen kysely, joten pyyntö, joka ei sisällä db-sarakkeita, onnistuu nyt.
      * Parannettu linkkien havaitseminen ja automaattinen linkki infotekstiin.

*    **Asioita ERDDAP™ Hallitsijoiden on tiedettävä ja tehtävä:** 
      * Parannettu tuki monimutkaiselle regexille, mukaan lukien useita kansiotasoja ja useita villikortteja.
      * Päivitetty riippuvuus.

## Versio 2.30.0{#version-2300} 
 (Lähde: 2026-04-07) 

Versio v2.30.0 keskittyy suurelta osin vikakorjauksiin, vakaus- ja turvallisuusriippuvuuspäivityksiin sekä suorituskyvyn parannuksiin.

*    **Uudet ominaisuudet ja muutokset (Käyttäjille) :** 
      * Parannettu [Croissant](https://mlcommons.org/working-groups/data/croissant/) metadatayhteensopivuus ja ilmeinen tuki, mukaan lukien [mlcroissant](https://pypi.org/project/mlcroissant/) yhteensopivuus.
      * Parannettu tuki parquet booleans.

*    **Asioita ERDDAP™ Hallitsijoiden on tiedettävä ja tehtävä:** 
      * Käyttämättömät komentorivityökalut ja niihin liittyvät koodit poistettiin koodikannasta teknisten velkojen vähentämiseksi. Näytä https://github.com/ERDDAP/erddap/pull/432.
 
      * Uusi lippu `SynchronousLoading` on lisätty ylikuormittamaan oletusarvoisesti siirrettyjen tietojen latausmenetelmä. Tämä on harvoin tarpeen, ja sitä käytetään vain tilanteissa, joissa lykätty lataus aiheuttaa ongelmia. Nähdään [Lippu sivut](/docs/server-admin/feature-flags#forcesynchronousloading) yksityiskohtiin.
...
Näiden henkilöiden/linkkien ansiosta ideoita ja näytekoodia vain CSS-kuvatyökaluille (oli https://codepen.io/electricalbah/pen/eJRLVd ) lykätty kuvan lataus (oli https://varvy.com/pagespeed/defer-images.html )   (koodia on muutettu ennen käyttöä ERDDAP ) .
...
Voit käyttää nimeä "\\*"Koko verkkotunnus, esim.\\*@example.com.
    * Koska kirjautuminen on muuttunut v1.66, lokitiedosto ei ole koskaan ajan tasalla. On aina viestejä tai viestejä, jotka odottavat kirjautumista lokitiedostoon. Voit tehdä sen ajan tasalla (hetkeksi) katsomalla sinun ERDDAP Tilan verkkosivuilla http://_your.domain.org_/erddap/status.html .
    * HashDigest...
...
_some.domain.name_
    * IngeneratDatasets.xml, for etätiedot (Esimerkkinä kolme palvelinta) automaattisesti luotu datasetID Ne ovat muuttumattomia useimmille verkkotunnuksille. Muutama verkkotunnus, ensimmäinen osa (a. Nimi) automaattisesti tuotettu datasetID tulee olemaan vähän erilainen. Nimillä, joilla oli yksi osa, on nyt kaksi osaa. Esimerkiksi tietoaineistot http://oos.soest.hawaii.edu Aiemmin johti datasetID Havaijilla aloitettu, mutta nyt datasetID Alkuperäisnimi Hawaiii's Theest. Jos tämä aiheuttaa ongelmia sinulle, lähetä minulle sähköpostia. Siellä voi olla työpaikka.
