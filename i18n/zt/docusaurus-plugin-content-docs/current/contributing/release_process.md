...
---
#  ERDDAP™ 釋放流程
* 確保影像比對檔案可用 (這可能意味著逃跑 `mvn 驗證` 如果您要加快速度, 只限影像比對群組, 雖然需要做 Jetty 測試) 
* 更新依赖性
```
...
```
* 執行測試, 確保所有主要設定的依赖性更新沒有損壞任何東西 (數據集特別解析, 雖然其他重要設定) . 注意 外部的測試套件可能很薄 慢AWS測試套件需要很長的時間
```
mvn verify
mvn verify -P external
mvn verify -P slowAWS
```
* 使用 `Python 翻譯/ 翻譯. py` 需要時更新翻譯。
...
cp ./内容/erddap/erddap2.22. war 或不管數字是多少
  * 在 \\[ 湯姆卡 \\] /bin/:
/啟動.sh
  * 之后 ERDDAP 已傳回網頁, in \\[ 湯姆卡 \\] /webapps/ :
...
* 刪除 i18n/ en 目錄 (需要是因為寫入翻譯不會覆寫已存在的檔案) 跑 `npm 執行寫入翻譯` 以匯出新 foller 字串。
* 編輯文件頁面 (部署/安装/更新/部署) .
  * 搜尋 \\[ 戰爭 \\]  
  * 复制已有信息 (稍稍重排) 至前一個裝置清單。
