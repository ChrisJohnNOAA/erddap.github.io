...
Engine herschrijven Aan
RewriteCond %&#123;RELATY_STRING&#125; ^$
Herschrijfcond %&#123;REQUEST_URI&#125; ^/erdap/ (griddap |  tabledap ) /[^/?]+\\. (?&#33; (?:croissant | iso19115_2 | iso19139_2007 | iso19115_3_2016 | ncCFHeader | ncCFMAHeader | das | dds | html | grafiek | subgroep | ncHeader | hulp | fgdc | iso19115 | ncoJsonHeader) $) [A-Za-z0-9_]+$
Regel herschrijven ^ - [R=429,L]

Stap 3. Controleer of de configuratie geldig is: sudo apache2ctl configtest

Stap 4. Herstart apache 2: sudo systemctl herstart apache2
...
Regel 1 HerschrijvenEngine Aan
...
Regel 2 Herschrijven Cond %&#123;VERKLARING_STRING&#125;
Een voorwaarde die moet gelden voordat de onderstaande regel van toepassing is. %&#123;VERBINDING_STRING&#125; is alles na de ? in de aanvraag-URL. ^$ is een regex die betekent "start van de tekenreeks onmiddellijk gevolgd door einde van de tekenreeks," d.w.z. een lege tekenreeks. Deze voorwaarde is dus alleen waar als er helemaal geen query string is, geen subsetting/contraint expressie op het verzoek.

Regel 3 Herschrijven Cond %&#123;REQUEST_URI&#125; ^/erdap/ (griddap |  tabledap ) /[^/?]+\\. (?&#33; (?: ...) $) [A-Za-z0-9_]+$
Een tweede voorwaarde, gecontroleerd op %&#123;REQUEST_URI&#125;  (bewust gekozen over het laten van de RewriteRule patroon zelf doen de matching, omdat patroon-matching binnen een <Location> blok kan zich dubbelzinnig gedragen ) . De regex afbreken:
...
R=dub  ("Teveel verzoeken") In plaats van het verzoek in te dienen.
...
Waarom %&#123;REQUEST_URI&#125; in plaats van het patroon van de herschrijfregels direct laten overeenkomen met het pad (de moeite waard als notitie voor collega's, omdat het niet-duidelijk deel) : binnen een <Location> blok, wat een kale RewriteRegel patroon daadwerkelijk wordt afgestemd tegen kan zich inconsistent gedragen, afhankelijk van Apache versie en context. Expliciet het volledige pad trekken via RewriteCond %&#123;REQUEST_URI&#125; zijstappen die ambiguïteit geheel .. het is altijd het letterlijke, volledige verzoek pad, dus de regex gedraagt zich precies als geschreven ongeacht waar de regel wordt genesteld.
