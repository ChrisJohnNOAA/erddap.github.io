...
PřepsatMotor Na
PřepsatCond % &#123;QUERY_STRING&#125; ^$
PřepsatCond % &#123;REQUEST_URI&#125; ^/erddap/ (griddap |  tabledap ) /[^/?] +\\. (?&#33; (?:croissant | iso19115_2 | iso19139_2007 | Iso19115_3_2016 | ncCFheader | ncCFMAHeader | das | dds | html | graf | Podkategorie | ncheader | Pomoc | fgdc | iso19115 | ncoJsonHeader) $) [A-Za-z0-9_]+$
Přepsat pravidlo ^ - [R=429,L]

Krok 3. Zkontrolujte, zda je konfigurace platná: sudo apache2ctl configtest

Krok 4. Restartovat apache2: sudo systemctl restartovat apache2
...
Čára 4 Přepsat pravidlo ^ - [R=429,L]
...
R=429 REAKCE S HTTP přesměrováním třídy s kódem stavu 429 ("Moc žádostí") místo podání žádosti.
...
Proč% &#123;REQUEST_URI&#125; místo toho, aby se vzor Přepisování shoduje přímo s cestou (stojí za to zahrnout jako poznámku pro kolegy, protože je to ne-zjevná část) : uvnitř <Location> block, co bare RewriteRule vzor skutečně dostane zápas proti může chovat nekonzistentní v závislosti na Apache verzi a kontext. Explicitně táhne celou cestu přes RewriteCond %&#123;REQUEST_URI&#125; bočnicemi, které zcela nejednoznačností, je to vždy doslovná, úplná cesta žádosti, takže regex se chová přesně tak psané bez ohledu na to, kde je pravidlo hnízdeno.
