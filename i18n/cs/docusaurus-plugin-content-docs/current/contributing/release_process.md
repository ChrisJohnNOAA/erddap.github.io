...
```
* Použití `python překlad/translate.py` v případě potřeby aktualizovat překlady.
...
* Smazat adresář i18n/en (potřebné, protože write-translations nepřepíše existující soubor) a utíkej `npm spustit write-translations` exportovat nový zápatí řetězec.
* Upravit stránky dokumentace (nasadit-install.md a nasadit-update.md) .
  * Hledat \\[ erddap.war \\]  
  * Kopírovat existující informace (mírně reformované) na seznam předchozích zařízení.
  * Změnit aktuální informace o vydání pro erddap. válka \\[ erddap.war \\] 
* Spusťte překlady na místo dokumentace. Doporučuje se pouze přeložit stránky, které se změnily, protože tento krok může být velmi pomalý.
  * Ujistěte se, že zápatí jsou přeloženy s novým číslem verze.
