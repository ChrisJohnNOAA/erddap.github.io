...
* Переконайтеся, що файли порівняння зображень доступні (це може означати хід `mvn перевірка` , якщо ви хочете прискорити обмеження, щоб просто група ImageComparison хоча зауважити, що все ще вимагає тестування Jetty) 
...
```
* Зареєструватися `python translation/translate.py` щоб оновити переклади, якщо необхідно.
...
з ./content/erddap/erddap2.22.war erddap.war/або будь-який номер
  * У \\[ домка \\] Українська
./startup.sh
  * Після ERDDAP повернула веб-сторінку, в \\[ домка \\] Веб-додатки / :
хрп -Р ерддап ерддап
...
* Видалити каталог i18n/en (потрібно, тому що записування не перезаписувати існуючий файл) і запустити `npm запустити записи` експортувати новий рядок нижнього колонтитула.
* Редагування сторінок документації (JavaScript licenses API Веб-сайт Go1.13.8) й
  * Пошук \\[ ерддап.воєнний \\]  
  * Скопіювати існуючу інформацію (трохи переформатований) до переліку попередніх установок.
  * Зміна інформації про поточний випуск для erddap. війни \\[ ерддап.воєнний \\] 
* Запустити переклади на сайт документації. Рекомендується лише перевести сторінки, які змінилися з цього кроку, можуть бути дуже повільно.
  * Переконайтеся, що ноги переведені з новим номером версії.
...
Якщо ви є ERDDAP адміністратор, інструкція по модернізації
