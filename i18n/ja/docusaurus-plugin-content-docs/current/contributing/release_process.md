...
* i18n/enディレクトリを削除します。 (write-translations が既存のファイルを上書きしないので必要) 実行 `npm 実行書き込み翻訳` 新しいフッター文字列をエクスポートします。
* ドキュメントページを編集する (deploy-install.md と deploy-update.md) お問い合わせ
  * お問い合わせ \\[ erddap.warの \\]  
  * 既存の情報をコピーする (少し改良された) 以前のインストールの一覧へ。
  * erddap の現在のリリース情報を変更します。 戦争で \\[ erddap.warの \\] 
* ドキュメントサイトの翻訳を実行します。 このステップは非常に遅くなる可能性があるので、変更したページのみを翻訳することをお勧めします。
  * 新しいバージョン番号でフッターが翻訳されていることを確認してください。
