...
* Βεβαιωθείτε ότι τα αρχεία σύγκρισης εικόνας είναι διαθέσιμα (Αυτό μπορεί να σημαίνει τρέξιμο. `mvn επαλήθευση` , αν θέλετε να επιταχύνετε ότι μέχρι να περιορίσει μόνο την ομάδα ImageComparison αν και σημειώστε ότι εξακολουθεί να απαιτεί τη διεξαγωγή δοκιμών Jetty) 
...
```
* Εκτέλεση δοκιμών για να βεβαιωθείτε ότι οι ενημερώσεις εξάρτησης δεν έσπασε τίποτα για όλες τις σημαντικές διαμορφώσεις (Τα σύνολα δεδομένων αναλύουν ειδικότερα, αν και άλλες σημαντικές ρυθμίσεις επίσης) . Σημειώστε ότι η εξωτερική σουίτα δοκιμής μπορεί να είναι πολύ flashy. Η δοκιμαστική σουίτα slowAWS μπορεί να διαρκέσει πολύ.
```
mvn verify
mvn verify -P external
mvn verify -P slowAWS
```
* Χρήση `μετάφραση python/translate.py` να ενημερώσει τις μεταφράσεις εάν χρειάζεται.
...
ιπ ../περιεχόμενο/erddap/erddap2.22.war erddap.war //ή οποιοδήποτε είναι ο αριθμός
  * Μέσα \\[ τομκάτ \\] /bin/:
./έναρξη.sh
  * Μετά την ERDDAP έχει επιστρέψει μια ιστοσελίδα, σε \\[ τομκάτ \\] /webapps/:
...
* Διαγραφή του καταλόγου i18n/el (απαιτείται επειδή οι μεταφράσεις εγγραφής δεν θα αντικαταστήσουν ένα υπάρχον αρχείο) και να τρέξει `npm run write-translatations` για την εξαγωγή της νέας συμβολοσειράς.
* Επεξεργασία των σελίδων τεκμηρίωσης (ανάπτυξη-εγκατάσταση.md και ανάπτυξη-update.md) .
  * Αναζήτηση για \\[ Erddap.war (στα Αγγλικά). \\]  
  * Αντιγραφή των υφιστάμενων πληροφοριών (ελαφρώς αναμορφωμένη) στον κατάλογο των προηγούμενων εγκαταστάσεων.
  * Αλλάξτε τις τρέχουσες πληροφορίες για την έκδοση erddap. πόλεμο \\[ Erddap.war (στα Αγγλικά). \\] 
* Εκτελέστε τις μεταφράσεις για το site τεκμηρίωσης. Συνιστάται να μεταφράζουν μόνο σελίδες που έχουν αλλάξει αφού αυτό το βήμα μπορεί να είναι πολύ αργό.
  * Βεβαιωθείτε ότι οι υποσέλιδοι έχουν μεταφραστεί με το νέο αριθμό έκδοσης.
...
Πρώτα ειδοποιήστε τυχόν χρήστες που ζήτησαν αλλαγές (ή των οποίων τα έντομα διορθώθηκαν) . Δώστε τους χρόνο να επαληθεύσουν τις αλλαγές ή/και να εγείρουν ζητήματα.
 ...
 ERDDAP Η έκδοση 2.25 είναι τώρα διαθέσιμη&#33;
...
Εάν είστε ERDDAP διαχειριστή, οι οδηγίες για την αναβάθμιση είναι στο
 https://erddap.github.io/docs/server-admin/deploy-update
 

Αν έχετε προβλήματα, ερωτήσεις, προτάσεις, παρακαλώ στείλτε μου email.

Σας ευχαριστώ για τη χρήση ERDDAP .
