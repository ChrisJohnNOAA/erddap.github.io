...
```
* Use Use Use Use Use `python çeviri/translate.python çeviri/translate.python` Gerekirse çevirileri güncellemek.
...
* I18n/en directory (Gerekli çünkü yazma-dönüşümleri mevcut bir dosyayı yazmayacaktır) ve koşmak `npm Write-translations` Yeni ayak dizesini ihraç etmek için.
* Belge sayfaları (Dağıtım.md ve dağıtım tarihi.md) .
  * Arama için \\[ Heddap.war \\]  
  * Mevcut bilgileri kopyalayın (Biraz reformcu) Önceki yüklemelerin listesine.
  * Mevcut sürüm bilgilerini erddap için değiştirin. Savaşta Savaş \\[ Heddap.war \\] 
* Belge sitesi için çevirileri çalıştırın. Bu adım çok yavaş olabileceğinden beri değişmiş olan sayfaları çevirmek önerilir.
  * Ayaklayıcıların yeni sürüm numarası ile tercüme olduğundan emin olun.
