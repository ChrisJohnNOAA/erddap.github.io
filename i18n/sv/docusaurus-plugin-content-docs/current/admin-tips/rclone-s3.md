...
Ett sätt att göra detta är att använda "rclone". ( https://rclone.org/ ) rclone fungerar på många olika S3-system, och har många olika inställningar för att optimera prestanda, inklusive att ställa in en cache-storlek, vilket förhoppningsvis kan kompensera några av hastighetsstraffet från att köra FUSE. Fördelen med att använda rclone med ERDDAP™ rclone hanterar all interaktion med S3, så datasettyper som EDDGrid FrånNcFiles kan användas direkt som om det finns lokala filer. Detta innebär att du bara behöver räkna ut hur du ställer in klon för att komma åt din objektbutik, och resten är bara vanliga Linux-typinställningar.
