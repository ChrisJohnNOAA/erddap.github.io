...
*    **Fitur dan Perubahan Baru (untuk pengguna) Sitemap** 
      * New ncoJson sebagai format output info yang tersedia sebagai file yang disebut nco JsonHeader dan di kedua /erddap/info/dataset Login .nc oJson dan /erddap/info/datasetId/index .nc Login
Terima kasih kepada @srstsavage
      * Sifat metadata tambahan untuk output ISO19139_2007 dan ISO19115_3_2016: klasifikasi, topik_kategori, dan pemeliharaan_frekuensi
Sitemap
      * Layanan web JSON baru untuk saran atribut variabel: /erddap/suggestVariableAttributes
Terima kasih kepada @srstsavage
      * Peningkatan querying dari sql backed dataset sehingga permintaan yang tidak mengandung kolom db sekarang akan berhasil.
      * Peningkatan deteksi tautan dan tautan otomatis untuk tautan dalam teks info.

*    **Sitemap ERDDAP™ Administrator Perlu Tahu dan Lakukan:** 
      * Peningkatan dukungan untuk regex jalur kompleks termasuk beberapa tingkat folder dan beberapa wildcards.
      * Ketergantungan yang diperbarui.

## Versi 2.30.0{#version-2300} 
 (dirilis 2026-04-07) 

Versi v2.30.0 sebagian besar berfokus pada perbaikan bug, pembaruan ketergantungan untuk stabilitas dan keamanan, dan peningkatan kinerja pengujian.

*    **Fitur dan Perubahan Baru (untuk pengguna) Sitemap** 
      * Sitemap [Login](https://mlcommons.org/working-groups/data/croissant/) kompatibilitas metadata dan dukungan manifest, termasuk [Login](https://pypi.org/project/mlcroissant/) Sitemap
      * Peningkatan dukungan untuk boolean parket.

*    **Sitemap ERDDAP™ Administrator Perlu Tahu dan Lakukan:** 
      * Unused command line tools dan kode terkait dihapus dari codebase untuk mengurangi utang teknis. Sitemap https://github.com/ERDDAP/erddap/pull/432.
 
      * Bendera fitur baru `Login` telah ditambahkan untuk menimpa pendekatan pemuatan dataset default. Ini harus jarang diperlukan, dan hanya digunakan dalam kasus di mana pemuatan yang rusak menyebabkan masalah. Sitemap [fitur halaman bendera](/docs/server-admin/feature-flags#forcesynchronousloading) Sitemap
...
Berkat orang / tautan ini untuk ide dan kode sampel untuk template alat gambar CSS-only (Sitemap https://codepen.io/electricalbah/pen/eJRLVd ) dan pemuatan gambar yang rusak (Sitemap https://varvy.com/pagespeed/defer-images.html )   (meskipun kode dimodifikasi sebelum digunakan ERDDAP ) Sitemap
...
Anda dapat menggunakan nama "\\*" untuk daftar hitam seluruh domain, misalnya,\\*Sitemap
    * Karena perubahan sistem penebangan dalam v1.66, file log tidak pernah terbaru. Selalu ada pesan atau bagian pesan yang menunggu untuk ditulis ke file log. Sekarang, Anda dapat membuatnya terbaru (Sitemap) dengan melihat ERDDAP 's status halaman web di http://_your.domain.org_/erddap/status.html Sitemap
    * Login
    ...
    * Sitemap Disarankan: berhenti menggunakan&lt;Login Sitemap datasets.xml untuk mengkonversi nomor IP dalam dataset&lt; sourceUrl Sitemap (Login http://192.168.#.#/ ) nama domain (Login http :my.domain.org/) Sitemap Mulai sekarang, berlangganan baru hingga http://localhost Login http://127.0.0.1 Sitemap http://192.168.#.# URL tidak akan diperbolehkan untuk alasan keamanan. Jadi, selalu menggunakan nama domain publik di Meme it&lt; sourceUrl Sitemap (jika diperlukan karena masalah DNS) Anda dapat menggunakan [/etc/hosts tabel di server Anda](https://linux.die.net/man/5/hosts) untuk memecahkan masalah dengan mengkonversi nama domain lokal ke nomor IP tanpa menggunakan server DNS. Anda dapat menguji apakah nama domain tertentu diselesaikan dengan benar dengan menggunakan
ping _some.domain.name_
    * Dalam menghasilkanDataset.xml, untuk dataset jarak jauh (Web server) , dihasilkan secara otomatis datasetID s tidak berubah untuk sebagian besar domain. Untuk beberapa domain, bagian pertama (i.e., nama) secara otomatis dihasilkan datasetID akan sedikit berbeda. Meme it Tidak mungkin, nama yang memiliki satu bagian sekarang lebih cenderung memiliki dua bagian. Sebagai contoh, dataset dari http://oos.soest.hawaii.edu sebelumnya menyebabkan datasetID s yang dimulai dengan hawaii\\_, tapi sekarang mengarah ke datasetID hawaii\\_soest\\_ Jika ini menyebabkan masalah bagi Anda, silakan email saya. Mungkin ada pekerjaan.
