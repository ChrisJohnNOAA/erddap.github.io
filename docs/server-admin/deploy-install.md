...
* On Linux and Mac, if you can't reach Tomcat or ERDDAP™ (or perhaps you just can't reach them from a computer outside your firewall),
  ...
  (where `#` is some digit), indicating that a `java` process (presumably Tomcat) is listening on port "8080" for "tcp" traffic.
...
3.  [Set up the `tomcat/content/erddap` configuration files.](#erddap-content)
    On Linux, Mac, and Windows, download [erddapContent.zip](https://github.com/ERDDAP/erddapContent/releases/download/content1.0.1/erddapContent.zip)
    and unzip it into the `tomcat` directory, creating `tomcat/content/erddap`.

    __Version 1.0.1, 20683 bytes, MD5=98a8099e7e674da59fe35e9c96efa7b5, dated 2025-06-02__
...
4. On Linux, Mac, and Windows, __download [erddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.31.0/ERDDAP-2.31.0.war)__ into `tomcat/webapps`:

   __Version 2.31.0, 719,567,541 bytes, MD5=DB0AFDE1F441CA9262CB66291A1110D7, dated 2026-08-28__
